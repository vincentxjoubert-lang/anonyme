...
run_user() {
    ...
    runuser -u "$TARGET_USER" -- env \
        HOME="$TARGET_HOME" USER="$TARGET_USER" LOGNAME="$TARGET_USER" "$@" || rc=$?
    if ((rc != 0)); then
        printf 'Commande utilisateur echouee:' >&3
        printf ' %q' "$@" >&3
        printf ' (code %s)\n' "$rc" >&3
        return "$rc"
    fi
...
install_available \
    rsync unzip xdg-utils xdg-user-dirs dbus-user-session \
...
theme_checkout() {
    local repo=$1 sha=$2 dir=$3 archive
    archive="$(mktemp --suffix=.tar.gz)"
    TEMP_PATHS+=("$archive")
    curl --fail --silent --show-error --location \
        --retry 3 --connect-timeout 20 \
        --output "$archive" \
        "https://codeload.github.com/vinceliuice/${repo}/tar.gz/${sha}"
    install -d "$dir"
