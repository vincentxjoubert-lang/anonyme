...
dpkg-query -W -f='${Status}\n' lubuntu-desktop 2>/dev/null | grep -q 'install ok installed' \
    || fail "une installation Lubuntu 26.04 complete est requise"
[[ -d /sys/firmware/efi ]] || fail "demarrage UEFI requis; le mode BIOS/CSM n'est pas accepte"

TARGET_USER="${SUDO_USER:-}"
[[ -n $TARGET_USER && $TARGET_USER != root ]] \
    || fail "lancez le script depuis l'utilisateur de bureau avec: sudo bash ./lubuntu-26.04-postinstall.sh"
...
install_available() {
    ...
    for pkg in "$@"; do
        if apt-cache show "$pkg" >/dev/null 2>&1; then
            ...
            fail "paquet requis indisponible dans les depots actifs: $pkg"
        fi
    done
    ((${#available[@]} == 0)) || apt_get install -y --no-install-recommends "${available[@]}"
}
...
/var/log/lubuntu-26.04-postinstall.log /var/log/lubuntu-26.04-hardware-validation.log {
    monthly
    rotate 4
    size 5M
    missingok
...
[[ $root_free_kib =~ ^[0-9]+$ ]] || fail "espace disque libre impossible a determiner"
((root_free_kib >= 8388608)) || fail "8 Gio libres minimum sont requis sur la partition racine"
root_source="$(findmnt -n -o SOURCE /)"
if ! lsblk -sno TYPE "$root_source" 2>/dev/null | grep -qx crypt; then
    warn "la partition systeme ne semble pas chiffree par LUKS; le chiffrement complet doit etre choisi pendant l'installation pour une confidentialite maximale"
fi
...
install_available \
    ...
    pciutils usbutils dmidecode ethtool nvme-cli smartmontools stress-ng bind9-dnsutils \
...
grep -qi 'AMD Ryzen 7 7800X3D' /proc/cpuinfo \
    || fail "processeur attendu AMD Ryzen 7 7800X3D non detecte"
if ! lspci -nn | grep -Eqi 'VGA|Display' || ! lspci -nn | grep -Eqi 'RX 6950 XT|Navi 21'; then
    fail "Radeon RX 6950 XT/Navi 21 attendue mais non detectee"
fi
mem_kib="$(awk '/^MemTotal:/ {print $2}' /proc/meminfo)"
((mem_kib >= 30000000)) || fail "au moins 30 Gio de RAM utilisable sont requis pour ce profil 32 Gio"

board_name="$(dmidecode -s baseboard-product-name 2>/dev/null || true)"
[[ $board_name == *'MAG B650 TOMAHAWK WIFI'* ]] \
    || fail "carte mere MSI MAG B650 TOMAHAWK WIFI non confirmee par DMI: ${board_name:-inconnue}"
lspci -nn | grep -Eqi 'RTL8125|Ethernet.*\[10ec:8125\]' \
    || fail "controleur Ethernet Realtek RTL8125BG 2.5 Gb/s non detecte"
nvme list 2>/dev/null | grep -qi 'CT1000P3SSD8' \
    || fail "SSD Crucial P3 1 To CT1000P3SSD8 non detecte"
if ! dmidecode -t 17 2>/dev/null | grep -Eq 'Configured Memory Speed:[[:space:]]+6000 MT/s'; then
    warn "DDR5-6000 non confirmee: activez EXPO puis validez la RAM avec Memtest86+"
fi
warn "la puissance du bloc d'alimentation, le refroidissement CPU et le flux d'air ne sont pas detectables par Linux; utilisez au minimum une alimentation 850 W de qualite et un refroidissement adapte"
...
EOF
    ...
    while IFS= read -r liquorix_image; do
        ...
        sbverify --cert "$mok_dir/MOK.pem" "$liquorix_image" >/dev/null 2>&1 \
            || fail "signature Secure Boot absente sur $liquorix_image"
    ...
    warn "Secure Boot est desactive dans l'UEFI; Liquorix fonctionnera, mais la chaine de confiance au demarrage reste inactive"
fi
...
if dpkg-query -W -f='${Status}' mullvad-vpn 2>/dev/null | grep -q 'install ok installed'; then
    systemctl enable --now mullvad-daemon.service
    systemctl is-active --quiet mullvad-daemon.service \
        || fail "service Mullvad installe mais inactif"
fi
...
systemctl is-active --quiet apparmor.service || fail "service AppArmor inactif"
aa-enabled || fail "AppArmor n'est pas active dans le noyau"
aa-status --enabled || fail "AppArmor n'est pas en etat active"
...
BACKSPACE="guess"
EOF
localectl set-x11-keymap fr pc105 legacy caps:shiftlock
...
Exec=$TARGET_HOME/.local/bin/validate-desktop-profile
OnlyShowIn=LXQt;
X-LXQt-Need-Tray=false
EOF

cat >"$TARGET_HOME/.local/bin/validate-desktop-profile" <<'EOF'
#!/usr/bin/env bash
set -u
umask 077
state_dir="${XDG_STATE_HOME:-$HOME/.local/state}/lubuntu-postinstall"
mkdir -p "$state_dir"
exec >"$state_dir/desktop-validation.log" 2>&1
date --iso-8601=seconds
"$HOME/.local/bin/apply-display-profile" || true
printf '\n[session]\n'
printf 'type=%s desktop=%s wayland=%s display=%s\n' \
    "${XDG_SESSION_TYPE:-}" "${XDG_CURRENT_DESKTOP:-}" \
    "${WAYLAND_DISPLAY:-}" "${DISPLAY:-}"
printf '\n[pipewire]\n'
pw-metadata -n settings 0 2>/dev/null || true
printf '\n[portals]\n'
systemctl --user --no-pager status \
    xdg-desktop-portal.service xdg-desktop-portal-lxqt.service \
    xdg-desktop-portal-wlr.service 2>&1 || true
printf '\n[gpu]\n'
vulkaninfo --summary 2>/dev/null || true
glxinfo -B 2>/dev/null || true
printf '\n[gamemode]\n'
gamemoded -t 2>&1 || true
EOF
chmod 0755 "$TARGET_HOME/.local/bin/validate-desktop-profile"

write_root_file /usr/local/sbin/validate-hardware-profile 0755 <<'EOF'
#!/usr/bin/env bash
set -u
umask 077
exec >/var/log/lubuntu-26.04-hardware-validation.log 2>&1
date --iso-8601=seconds
printf '\n[kernel-secureboot]\n'
uname -a
...
grep -E 'MemTotal|SwapTotal' /proc/meminfo || true
dmidecode -t 0 -t 2 -t 17 2>/dev/null | grep -E \
    'Vendor:|Version:|Release Date:|Manufacturer:|Product Name:|Configured Memory Speed:' || true
printf '\n[pci-drivers-rebar]\n'
lspci -nnk
lspci -vv 2>/dev/null | grep -A12 -E 'VGA|Display|Resizable BAR' || true
printf '\n[network]\n'
while IFS= read -r interface; do
    ethtool "$interface" 2>/dev/null | grep -E 'Speed:|Duplex:|Auto-negotiation:|Link detected:' || true
    ethtool -i "$interface" 2>/dev/null || true
done < <(find /sys/class/net -mindepth 1 -maxdepth 1 -type l -printf '%f\n' | grep -vE '^(lo|wg|tun|tap)')
printf '\n[nvme]\n'
nvme list 2>/dev/null || true
while IFS= read -r device; do
    nvme smart-log "$device" 2>/dev/null || smartctl -a "$device" 2>/dev/null || true
    block="${device#/dev/}"
    [[ -r /sys/block/$block/queue/scheduler ]] && cat "/sys/block/$block/queue/scheduler"
done < <(nvme list 2>/dev/null | awk 'NR>2 && $1~/^\/dev\/nvme/ {print $1}')
printf '\n[temperatures]\n'
...
resolvectl status
printf '\n[sysctl]\n'
sysctl vm.swappiness vm.vfs_cache_pressure net.core.default_qdisc \
    net.ipv4.tcp_congestion_control net.core.rmem_max net.core.wmem_max 2>/dev/null || true
EOF

write_root_file /etc/systemd/system/lubuntu-hardware-validation.service <<'EOF'
[Unit]
Description=Validate the tuned gaming workstation after boot
After=multi-user.target NetworkManager.service

[Service]
Type=oneshot
ExecStart=/usr/local/sbin/validate-hardware-profile
...
systemd-analyze verify \
    /etc/systemd/system/radio-block.service \
    /etc/systemd/system/quad9-dot-refresh.service \
    /etc/systemd/system/quad9-dot-refresh.timer \
    /etc/systemd/system/lubuntu-hardware-validation.service
systemctl enable lubuntu-hardware-validation.service
...
if $secure_boot_enabled; then
    while IFS= read -r liquorix_image; do
        sbverify --cert /root/secureboot-liquorix/MOK.pem "$liquorix_image" >/dev/null 2>&1 \
            || fail "noyau Liquorix non signe: $liquorix_image"
    done < <(find /boot -maxdepth 1 -type f -name 'vmlinuz-*-liquorix-amd64' -print)
fi
...
/usr/local/sbin/validate-hardware-profile
run_user dbus-run-session gamemoded -t

while IFS=: read -r active_device active_type active_state; do
    [[ $active_type == ethernet && $active_state == connected ]] || continue
    link_speed="$(ethtool "$active_device" 2>/dev/null | awk -F: '/Speed:/ {gsub(/[[:space:]]/,"",$2); print $2}')"
    [[ $link_speed == 2500Mb/s ]] \
        || warn "liaison $active_device negociee a ${link_speed:-vitesse inconnue}, pas a 2.5 Gb/s"
done < <(nmcli --terse --fields DEVICE,TYPE,STATE device status)

dot_protocol="$(dig +time=3 +tries=1 +short txt proto.on.quad9.net 2>/dev/null | tr -d '"' || true)"
if [[ $dot_protocol != dot. ]]; then
    warn "Quad9 DoT n'a pas pu etre confirme maintenant (reponse: ${dot_protocol:-aucune}); le profil sera reapplique au demarrage"
fi
...
if $secure_boot_enabled && ! mokutil --test-key /root/secureboot-liquorix/MOK.der >/dev/null 2>&1; then
    printf 'Au redemarrage, validez Enroll MOK pour autoriser le noyau Liquorix signe.\n' >&3
fi
printf 'Termine. Redemarrez le PC. Journal: %s | Validation apres demarrage: /var/log/lubuntu-26.04-hardware-validation.log | Sauvegarde: %s\n' \
