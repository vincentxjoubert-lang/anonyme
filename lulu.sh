...
DO "Mesa + Vulkan"          pkg mesa-vulkan-drivers mesa-vulkan-drivers:i386 \
...
DO "install pipewire"       pkg pipewire pipewire-pulse pipewire-alsa wireplumber pavucontrol sof-firmware
...
DO "installation Thunar & greffons" pkg thunar thunar-archive-plugin thunar-volman thunar-media-tags-plugin \
...
DO "portails + Qt wayland" pkg xdg-desktop-portal xdg-desktop-portal-wlr xdg-desktop-portal-gtk qtwayland5 qt6-wayland xwayland

#===================================================== 10. GameMode (64 + 32 bits) ==
title "10. GameMode + support 32 bits"
DO "gamemode 64/32" pkg gamemode libgamemode0 libgamemode0:i386 libgamemodeauto0 libgamemodeauto0:i386
usermod -aG gamemode "$REAL_USER" >>"$LOG" 2>&1 || true

#========================================================= 11. Polices internationales ==
title "11. Polices : latin, CJK (中/日/한), arabe, emoji couleur"
DO "familles Noto + arabe" pkg fonts-noto-core fonts-noto-ui-core fonts-noto-extra \
...
DO "moteur Kvantum Qt6 + deps" pkg qt6-style-kvantum qt6-style-kvantum-themes git sassc libglib2.0-dev-bin optipng
TMP="$(mktemp -d)"; chown "$REAL_USER:$REAL_USER" "$TMP"
_git(){ as_user git clone --depth=1 "$1" "$2" >>"$LOG" 2>&1; }

step "thème GTK WhiteSur (dark)"
{ _git https://github.com/vinceliuice/WhiteSur-gtk-theme "$TMP/gtk" && as_user "$TMP/gtk/install.sh" -c Dark -l --silent-mode; } >>"$LOG" 2>&1 && ok || warn "GTK"

step "icônes WhiteSur (dark)"
{ _git https://github.com/vinceliuice/WhiteSur-icon-theme "$TMP/icons" && as_user "$TMP/icons/install.sh" -b; } >>"$LOG" 2>&1 && ok || warn "icônes"

step "curseur macOS (McMojave, système)"
{ _git https://github.com/vinceliuice/McMojave-cursors "$TMP/cursors" && "$TMP/cursors/install.sh"; } >>"$LOG" 2>&1 && ok || warn "curseur"
...
cat <<EOF
  ...
  ouvrir « Configuration LXQt » -> Apparence et vérifier :
    ...
    • Thème LXQt (panneau)  : kvantum          • Curseur : McMojave-cursors
