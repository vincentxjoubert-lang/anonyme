#!/usr/bin/env bash
#===============================================================================
#  fix-theme.sh — Thème macOS "Mojave Dark" complet pour Lubuntu 26.04 (LXQt/Qt6)
#  GTK: Mojave-Dark | Icônes: McMojave-circle-dark | Curseur: WhiteSur-cursors
#  Qt: Kvantum (McMojave) | Panneau LXQt: System | Fenêtres: Openbox macOS
#  Chaque installateur a été testé. Idempotent, non bloquant (stdin /dev/null).
#  Usage : sudo ./fix-theme.sh   puis DÉCONNEXION / RECONNEXION de session.
#===============================================================================
set -uo pipefail
...
pkg_loop(){ local p; for p in "$@"; do apt-get install -y --no-install-recommends "$p" >>"$LOG" 2>&1 || echo "PKG FAIL: $p" >>"$LOG"; done; }
_git(){ as_user git clone --depth=1 "$1" "$2" >>"$LOG" 2>&1; }

printf '%sThème macOS "Mojave Dark" — installation (user=%s)%s\n' "$(c '1;37')" "$REAL_USER" "$(c 0)"
printf 'Journal : %s\n\n' "$LOG"

step "dépendances (git, sassc, Kvantum Qt6)"
pkg_loop git sassc libglib2.0-dev-bin optipng qt6-style-kvantum qt6-style-kvantum-themes; ok

TMP="$(mktemp -d)"; chown "$REAL_USER:$REAL_USER" "$TMP"

step "thème GTK Mojave (dark)"
as_user rm -rf "$USER_HOME/.themes/Mojave"* "$USER_HOME/.local/share/themes/Mojave"* >/dev/null 2>&1
{ _git https://github.com/vinceliuice/Mojave-gtk-theme "$TMP/gtk" \
  && timeout 600 as_user bash -c "cd '$TMP/gtk' && bash install.sh -c dark"; } </dev/null >>"$LOG" 2>&1 && ok || warn "GTK"

step "icônes McMojave-circle (dark)"
as_user rm -rf "$USER_HOME/.local/share/icons/McMojave"* "$USER_HOME/.icons/McMojave"* >/dev/null 2>&1
{ _git https://github.com/vinceliuice/McMojave-circle "$TMP/icons" \
  && timeout 600 as_user bash -c "cd '$TMP/icons' && bash install.sh"; } </dev/null >>"$LOG" 2>&1 && ok || warn "icônes"

step "curseur macOS WhiteSur (système)"
rm -rf /usr/share/icons/WhiteSur-cursors >/dev/null 2>&1
{ _git https://github.com/vinceliuice/WhiteSur-cursors "$TMP/cursors" \
  && ( cd "$TMP/cursors" && timeout 300 bash install.sh ); } </dev/null >>"$LOG" 2>&1 && ok || warn "curseur"

step "thème Qt/Kvantum McMojave (dark)"
if _git https://github.com/vinceliuice/McMojave-kde "$TMP/kde" >>"$LOG" 2>&1; then
  KV="$USER_HOME/.config/Kvantum"; /usr/bin/install -d -o "$REAL_USER" -g "$REAL_USER" "$KV"
  cp -r "$TMP"/kde/Kvantum/McMojave "$KV"/ 2>>"$LOG"
...
OBDIR="/usr/share/themes/Mojave-Dark-OB/openbox-3"; mkdir -p "$OBDIR"
cat > "$OBDIR/themerc" <<'EOF'
! Mojave-Dark-OB — décoration Openbox sombre, accents macOS
border.width: 1
padding.width: 7
padding.height: 5
...
# Panneau LXQt = System (suit Kvantum, recommandé par la doc LXQt) + style Qt Kvantum
uconf "$USER_HOME/.config/lxqt/lxqt.conf" <<'EOF'
[General]
icon_theme=McMojave-circle-dark
theme=system
...
cat <<EOF
  Résultat attendu : fenêtres, panneau, applis Qt et GTK en macOS sombre.
  Si un élément ne s'applique pas seul -> Configuration LXQt -> Apparence :
    Style des widgets = kvantum-dark | Icônes = McMojave-circle-dark | Thème LXQt = System
  Bords de fenêtres : Préférences -> Openbox -> Theme -> Mojave-Dark-OB
  Erreurs éventuelles : grep WARN $LOG   |   échecs de paquet : grep 'PKG FAIL' $LOG
EOF
