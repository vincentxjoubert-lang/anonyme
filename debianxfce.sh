...
readonly BACKUP_DIR="/root/debian13-setup-backup-$(date +%Y%m%d-%H%M%S)"
...
msg "Steam et compatibilité 32 bits"
...
EOF
    ...
    if ! apt-get update || ! apt_install codium; then
        rm -f "$source" "$keyring"
        apt-get update -qq || true
        return 1
    fi
...
EOF
    ...
    if ! apt-get update || ! apt_install virtualbox-7.2; then
        rm -f "$source" "$keyring"
        apt-get update -qq || true
        return 1
    fi
