...
audit_system() {
    local failures=0 warnings=0 dpkg_audit
    : >"$AUDIT_FILE"
    ...
    for package in brave-browser code virtualbox-7.2 steam-installer qalculate-qt \
        firmware-amd-graphics amd64-microcode; do
    ...
    if flatpak info com.discordapp.Discord >/dev/null 2>&1; then audit_ok "Discord Flatpak"; else audit_fail "Discord absent"; fi
    if flatpak info org.vinegarhq.Sober >/dev/null 2>&1; then audit_ok "Sober Flatpak"; else audit_fail "Sober absent"; fi
...
BASE_PACKAGES=(
    ...
    fonts-noto-core fonts-noto-color-emoji arc-theme papirus-icon-theme
...
#!/bin/sh
xrdb -merge "$TARGET_HOME/.Xresources"
setxkbmap -rules evdev -model pc105 -layout fr_numlock -variant basic -option ''
xsetroot -solid '#10131a'

"$TARGET_HOME/.local/bin/display-setup" &
xsettingsd --config "$TARGET_HOME/.config/xsettingsd/xsettingsd.conf" &
lxpolkit &
nm-applet &
blueman-applet &
dunst -config "$TARGET_HOME/.config/dunst/dunstrc" &
picom --config "$TARGET_HOME/.config/picom/picom.conf" &
tint2 -c "$TARGET_HOME/.config/tint2/tint2rc" &
gammastep -c "$TARGET_HOME/.config/gammastep/config.ini" &
flameshot &
...
as_user xdg-mime default brave-browser.desktop x-scheme-handler/http
as_user xdg-mime default brave-browser.desktop x-scheme-handler/https
as_user xdg-mime default brave-browser.desktop text/html
for mime in image/jpeg image/png image/gif image/webp image/bmp; do
    as_user xdg-mime default qimgv.desktop "$mime"
done
as_user xdg-mime default featherpad.desktop text/plain
