...
# Révision V6 auditée le 2026-08-04
...
audit_system() {
    ...
    if grep -Fqx 'panel_items = LTEEEESC' "$TARGET_HOME/.config/tint2/tint2rc" 2>/dev/null && \
        ...
        [[ $exec_count == 4 ]]; then
        audit_ok "Barre Tint2 moderne, collée et réservée"
    else
    ...
    if grep -Eq '^[[:space:]]*dns=none$' <<<"$nm_config" && \
        grep -Eq '^[[:space:]]*systemd-resolved=false$' <<<"$nm_config"; then
    ...
    for package in xserver-xorg xinit openbox obconf lightdm lightdm-gtk-greeter \
