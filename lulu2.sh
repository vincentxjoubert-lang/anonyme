...
{ _git https://github.com/vinceliuice/WhiteSur-gtk-theme "$TMP/gtk" && as_user bash -c "cd '$TMP/gtk' && bash install.sh -c Dark -l"; } >>"$LOG" 2>&1 && ok || warn "GTK"

step "icônes WhiteSur (dark)"
{ _git https://github.com/vinceliuice/WhiteSur-icon-theme "$TMP/icons" && as_user bash -c "cd '$TMP/icons' && bash install.sh -b"; } >>"$LOG" 2>&1 && ok || warn "icônes"

step "curseur macOS WhiteSur (système)"
{ _git https://github.com/vinceliuice/WhiteSur-cursors "$TMP/cursors" && ( cd "$TMP/cursors" && bash install.sh ); } >>"$LOG" 2>&1 && ok || warn "curseur"
